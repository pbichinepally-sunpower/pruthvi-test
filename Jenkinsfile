...
def bucketName='nightvision-artifacts'

def slack_channel = 'nightvision-build-status'
...
pipeline {
            ...
            steps {
                withCredentials([
                    ...
                    script{
                        ...
                        cd ./night-cli/ > /dev/null && skaffold build -p dev --default-repo=us.gcr.io/spwr-cd-90fd/nightvision --filename=skaffold.yml --file-output=night-cli-jumpbox-${commit}.json && cd .. > /dev/null
    ...
    post {
        ...
        always{
            sendNotifications currentBuild.result, "Job: '${env.JOB_NAME}', Branch-Commit: '${branch}-${commit}', BuildNumber: '${env.BUILD_NUMBER}'", slack_channel
        }
        success {
            script {
                    ...
                    def identity = awsIdentity()    
                        //s3Upload(bucket:"nightvision-artifacts", file: '*.json');
                            ...
                            s3Upload(bucket:"${bucketName}",
                                includePathPattern:"**/*",
                                workingDir:'night-cli',
                                metadatas:["commit-id:${commit}","jenkins-build-number:${env.BUILD_NUMBER}"])
                            s3Upload(
                                bucket:"${bucketName}",
                                includePathPattern:"**/*",
                                workingDir:'services',
